select * from (payment join paymenttype on payment.type_id = paymenttype.id)
where paymenttype.name = "MONTHLY";

select * from student;
select * from (subject inner join mark on subject.id = mark.id)
where subject.name = "Art";


select st.* from student as st join payment as p on st.id = p.student_id
